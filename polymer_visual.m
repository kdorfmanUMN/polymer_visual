...
function polymer_visual(filename,options)
    ...
    arguments
        ...
        % title specifies a string to use as the figure titles. Default is
        % none (""). Note that any title provided here will be used as the
        % title for *all* figures generated by this script.
        options.title = "";
        ...
        % If hide_colorbars is set to true, colorbars will not be drawn on
        % any plots
    ...
    [R,x,y,z,dim,lattype] = read_rgrid(filename,options.fieldId);
    ...
    if ~isfield(options,'max_comps')
        options.max_comps = zeros(1,n_mnr);
        for in = 1:n_mnr
            options.max_comps(in) = max(R(:,:,:,in),[],'all');
        end
    end
    ...
    % in mono_disp:
    individual_profiles(R,x,y,z,dim,...
    ...
    scattering_plot(R,x,y,z,...
                    'scatterers',options.scatterers,...
                    'fontsize',options.fontsize,...
                    'savefile',savefile_scat,...
                    'theta_plot',options.theta_plot,...
                    'units',options.units,...
                    'writefile',writefile_scat,...
                    'hkls',options.hkls,...
                    'film_params',options.film_params,...
                    'resolution',options.resolution,...
    ...
    line_profile(R,options.direc,options.startloc,...
                 'savefile',savefile_1D,...
                 'fontsize',options.fontsize,...
                 'mono_label',options.mono_label,...
                 'title',options.title,...
    ...
    contour_plot(R,options.contourvecs,basis,...
                 'fontsize',options.fontsize,...
                 'title',options.title,...
                 'mono_label',options.mono_label,...
                 'map',options.map,...
